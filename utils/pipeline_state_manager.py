...
class PipelineStateManager:
    ...
    def __init__(self, prompt: str):
        ...
        self.prompt_dir = settings.DEBUG_PATH / self.prompt_hash
        ...
        if self.state_file_path.exists():
            try:
                ...
                if loaded_state.get("prompt_hash") == self.prompt_hash:
                    ...
                    self.clear_all_state() # Clear the state in this specific directory
    ...
    def clear_all_state(self):
        """Deletes the main state file and all associated asset files for the current prompt hash."""
        self.logger.warning(f"Clearing all persisted pipeline state for prompt hash: {self.prompt_hash}")
        try:
            import shutil
            # Remove the entire prompt directory which contains all state and assets
            if self.prompt_dir.exists():
                shutil.rmtree(self.prompt_dir)
                self.logger.info(f"Successfully deleted prompt directory: {self.prompt_dir}")
            # The old logic for finding and deleting other asset files is now redundant.
            # The load_state will handle clearing unmatched prompt directories.
        except OSError as e:
            self.logger.error(f"Error deleting prompt directory {self.prompt_dir}: {e}")
